...
class mainGame extends Phaser.Scene {
  ...
  preload_files(on_complete) {
    ...
    for (let i = 0; i < preload_sounds.length; i++) {
      this.load.audio(
        preload_sounds[i],
        "assets/audio/" + preload_sounds[i] + ".mp3"
      );
    }
    ...
    if (loading_vars["js_combined"]) {
      ...
      this.load.script(
        "loading_overlay",
        "js/game_utilities/loading_overlay.js"
      );
      this.load.script(
        "graphics_manager",
        "js/game_utilities/graphics_manager.js"
      );
      this.load.script(
        "test_ad_manager",
        "js/game_utilities/test_ad_manager.js"
      );
      ...
      this.load.script(
        "purchase_success",
        "js/game_windows/purchase_success.js"
      );
  ...
  set_loading_progress(val) {
    if (this.load_progress_cont) {
      ...
      this.load_progress_cont = new Phaser.GameObjects.Container(
        this,
        loading_vars["W"] / 2,
        loading_vars["H"] / 2
      );
      this.add.existing(this.load_progress_cont);
      this.load_progress_txt = new Phaser.GameObjects.Text(
        this,
        0,
        -25,
        String(val) + "%",
      ...
      this.load_progress_bar = new Phaser.GameObjects.Image(
        this,
        0,
        19,
        "preload_bar"
      );
      this.load_progress_bg = new Phaser.GameObjects.Image(
        this,
        0,
        20,
        "preload_bg"
      );
      this.load_progress_bg2 = new Phaser.GameObjects.Image(
        this,
        0,
        20,
        "preload_bg2"
...
function pause() {
  ...
  if (
    game_data["game_windows"].game_window &&
    game_data["game_windows"].game_window.pause_timer &&
    typeof game_data["game_windows"].game_window.pause_timer === "function"
  ) {
    game_data["game_windows"].game_window.pause_timer();
  }
  console.log("pause");
...
function resume() {
  ...
  if (
    game_data &&
    game_data["game_play"] &&
    !game_data["game_play"].is_paused() &&
    !(game_data.game_windows && game_data.game_windows.game_window)
  ) {
  ...
  if (
    ...
    typeof game_data["game_windows"].game_window.resume_timer === "function"
  ) {
    game_data["game_windows"].game_window.resume_timer();
  }
  console.log("resume");
